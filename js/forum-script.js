function original() {
    ...
    // document.addEventListener('DOMContentLoaded', () => {
        // 獲取所有類別標籤
        const newscategories = document.querySelectorAll('.news_categories div');
        // 獲取所有內容區域
    ...
    // document.addEventListener('DOMContentLoaded', () => {
        ...
        // 取得所有內容區域
        const rightcontentSections = document.querySelectorAll('.body_atc > div');
        ...
        categoryButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                // 隱藏所有內容區域
                rightcontentSections.forEach(section => section.classList.remove('active'));
    ...
    // document.addEventListener('DOMContentLoaded', () => {
        // 获取所有类别标签
        const teachcategories = document.querySelectorAll('.teach_categories div');
        // 获取所有内容区域
        const teachcontentSections = document.querySelectorAll('.body_teach ul');
        // 获取唯一的 "更多" 链接按钮
        const moreLink = document.querySelector('.teachs_more-link');

        // 为每个类别标签添加点击事件
        teachcategories.forEach((category) => {
            category.addEventListener('click', () => {
                // 移除所有类别标签和内容区域的 active 类
                teachcategories.forEach(cat => cat.classList.remove('active'));
                teachcontentSections.forEach(section => section.classList.remove('active'));
